...
-- start with a copy of defaults to avoid accidental aliasing
MODULES = {}
...
-- Load user settings if they exist, merge intelligently
local settings_file = "settings.lua"
local f = io.open(settings_file, "r")
if f ~= nil then
  io.close(f)
  -- require settings (may overwrite globals, including MODULES)
  local ok, err = pcall(require, "settings")
  if not ok then
    print("[SketchyBar] Failed to load settings.lua: " .. tostring(err))
  end

  -- If user set MODULES to a table, merge defaults into it (so unspecified keep default)
  if type(MODULES) == "table" then
    ...
    -- If MODULES got replaced by something unexpected (e.g., nil), restore defaults
    MODULES = {}
    for k, v in pairs(DEFAULT_MODULES) do
      MODULES[k] = v
    end
